...
class AgenteOCR:
    ...
    def __init__(
        ...
        *,
        ...
        use_gpu: bool = False,
        use_aws_textract: Optional[bool] = None,
        ...
        # Integração AWS Textract (opcional)
        self._aws_enabled = False
        self._aws_client = None
        try:
            flag_env = (os.getenv("USE_AWS_TEXTRACT", "false").strip().lower() in {"1","true","yes"})
            self._aws_enabled = bool(use_aws_textract if use_aws_textract is not None else flag_env)
            if self._aws_enabled:
                ...
                try:
                    ...
                    pass
                self._aws_client = boto3.client("textract", region_name=region)
                log.info(f"AWS Textract habilitado (region={region}).")
        except Exception as e:
            self._aws_enabled = False
            self._aws_client = None
            log.warning(f"Falha ao habilitar AWS Textract: {e}")
    ...
    def reconhecer_com_meta(self, nome: str, conteudo: bytes) -> Tuple[str, Dict[str, Any]]:
        ...
        try:
            if ext == ".pdf":
                # Tenta AWS Textract primeiro (AnalyzeExpense) se habilitado
                if self._aws_enabled:
                    texto, conf, engine = self._aws_textract_expense(nome, conteudo)
                    if engine:
                ...
                # Tenta AWS Textract primeiro se habilitado
                if self._aws_enabled:
                    texto, conf, engine = self._aws_textract_expense(nome, conteudo)
                    if engine:
                        self.last_stats["ocr_engine"] = engine
    ...
    # ------------------------- AWS Textract -------------------------
    def _aws_textract_expense(self, nome: str, conteudo: bytes) -> Tuple[str, float, str]:
        """Usa AnalyzeExpense (sincrono) com Bytes para extrair campos e itens.
        Retorna (texto, conf, engine). Em falha, retorna ("", 0.0, "").
        """
        if not getattr(self, "_aws_enabled", False) or not getattr(self, "_aws_client", None):
            return "", 0.0, ""
        try:
            t0 = time.time()
            resp = self._aws_client.analyze_expense(Document={"Bytes": conteudo})
            lines: List[str] = []
            confs: List[float] = []
            docs = resp.get("ExpenseDocuments") or []
            for d in docs:
                for sf in d.get("SummaryFields", []):
                    label = ((sf.get("LabelDetection") or {}).get("Text") or "").strip()
                    val_det = (sf.get("ValueDetection") or {})
                    value = (val_det.get("Text") or "").strip()
                    if label or value:
                        lines.append(f"{label}: {value}" if label else value)
                    try:
                        c = float(val_det.get("Confidence") or 0.0)
                        if c:
                            confs.append(c / 100.0 if c > 1.0 else c)
                    except Exception:
                        pass
                for grp in d.get("LineItemGroups", []):
                    for it in grp.get("LineItems", []):
                        kv = { (x.get("Type") or "").upper(): (x.get("ValueDetection") or {}).get("Text") for x in it.get("LineItemExpenseFields", []) }
                        desc = (kv.get("ITEM") or kv.get("DESCRIPTION") or "").strip()
                        qty = (kv.get("QUANTITY") or "").strip()
                        price = (kv.get("PRICE") or kv.get("UNIT_PRICE") or "").strip()
                        amount = (kv.get("AMOUNT") or "").strip()
                        row = " | ".join([x for x in [desc, qty, price, amount] if x])
                        if row:
                            lines.append(row)
                        for k in ("QUANTITY","PRICE","UNIT_PRICE","AMOUNT"):
                            try:
                                c = float(((next((f for f in it.get("LineItemExpenseFields", []) if (f.get("Type") or "").upper()==k), {}) or {}).get("ValueDetection") or {}).get("Confidence") or 0.0)
                                if c:
                                    confs.append(c / 100.0 if c > 1.0 else c)
                            except Exception:
                                pass
            texto = "\n".join(lines).strip()
            conf = float(sum(confs)/len(confs)) if confs else 0.85
            if texto:
                self.last_stats["ocr_engine"] = "aws_textract_expense"
                # páginas podem ser inferidas por grupos, usa len(docs) como proxy
                self.last_stats["pages"] = int(len(docs) or 1)
                self.last_stats["ocr_blocks"] = int(len(lines))
                self.last_stats["avg_confidence"] = conf
                self.last_stats["native_source"] = "aws"
                log.info("Textract AnalyzeExpense '%s' em %.2fs", nome, (time.time()-t0))
                return texto, conf, "aws_textract_expense"

            # Fallback: usar AnalyzeDocument (FORMS/TABLES) quando Expense não retorna conteúdo
            t1 = time.time()
            resp2 = self._aws_client.analyze_document(
                Document={"Bytes": conteudo}, FeatureTypes=["FORMS", "TABLES"]
            )
            blocks = resp2.get("Blocks") or []
            line_texts: List[str] = []
            line_confs: List[float] = []
            max_page = 0
            for b in blocks:
                if (b.get("BlockType") or "").upper() == "LINE":
                    ...
                    if t:
                        ...
                        try:
                            ...
                            if c:
                                line_confs.append(c / 100.0 if c > 1.0 else c)
                        except Exception:
                            pass
                try:
            ...
            conf2 = float(sum(line_confs)/len(line_confs)) if line_confs else (conf or 0.80)
            if texto2:
                self.last_stats["ocr_engine"] = "aws_textract_document"
                self.last_stats["pages"] = int(max_page or 1)
                self.last_stats["ocr_blocks"] = int(len(line_texts))
                self.last_stats["avg_confidence"] = conf2
                self.last_stats["native_source"] = "aws"
                log.info("Textract AnalyzeDocument '%s' em %.2fs", nome, (time.time()-t1))
                return texto2, conf2, "aws_textract_document"

            # Sem conteúdo de ambos — retorna vazio para cair em fallback local
            log.info("Textract não retornou conteúdo útil para '%s' (Expense/Document)", nome)
            return "", 0.0, ""
        except Exception as e:
            log.debug(f"Textract AnalyzeExpense falhou: {e}")
            return "", 0.0, ""
