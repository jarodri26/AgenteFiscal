...
def ui_sidebar():
    with st.sidebar:
        ...
        with col_sv1:
            if st.button("Salvar View", width="stretch", disabled=(not sv_name.strip())):
        ...
        with col_sv2:
            if st.button("Limpar Views", width="stretch"):
...
def tab_auditoria(orch: Orchestrator, db: BancoDeDados):
    ...
    if abrir and doc_id > 0:
        ...
        with col_left:
            ...
            ]], width="stretch", height=220, disabled=True, key=f"doc_header_{doc_id}")
        ...
        with st.spinner("Analisando..."):
            try:
                ...
                if isinstance(answer_table, pd.DataFrame) and not answer_table.empty:
                    st.data_editor(tidy_dataframe(answer_table), width="stretch", height=360, disabled=True, key=f"ans_tbl_{len(history)}")
...
def tab_admin(db: BancoDeDados):
    ...
    with a1:
        try:
            ...
            with c3:
                if st.button("Deletar", disabled=(uid_del == 0 or not confirmar), width="stretch"):
                    if uid_del == 1:
