...
class Orchestrator:
    ...
    def __init__(
        ...
        aws_textract_flag = os.getenv("USE_AWS_TEXTRACT", "false").strip().lower() in {"1","true","yes"}
        aws_region = os.getenv("AWS_REGION") or os.getenv("AWS_DEFAULT_REGION")
        self.ocr_agent = AgenteOCR(llm=self.llm, use_aws_textract=aws_textract_flag, aws_region=aws_region)
    ...
    def _processar_midias(self, nome: str, conteudo: bytes, origem: str) -> int:
        ...
        try:
            ...
            if (texto or "").strip():
                ...
                self._merge_meta_json(doc_id, {"ocr": {"tipo": ocr_tipo, "conf": float(conf)}})
                try:
                    # Persistir ocr_meta também no modo determinístico
                    self._merge_meta_json(doc_id, {"ocr_meta": ocr_meta or {}})
                except Exception:
                    pass
